...
const { eventsListener, default: downloadHandler } = require("./download.js");
...
const getSnapshotsDir = (options) => {
  ...
  try {
    ...
    // console.error(e);
    Util.output(
      `Failed to create download dir: ${Util.formatPath(snapshotsDir)}`,
      true
    );
  }
  return snapshotsDir;
...
const statsInfoHandler = (options) => {
  ...
  if (executablePath) {
    ...
    executablePath = fs.existsSync(executablePath)
      ? EC.green(executablePath)
      : EC.red(executablePath);
  ...
  if (typeof options.launchable === "boolean") {
    ...
    const launchable = statsInfo.launchable
      ? EC.green("true")
      : EC.red("false");
...
const PCR = async (options) => {
  ...
  if (stats && fs.existsSync(stats.executablePath)) {
    ...
    if (
      !options.revision ||
      (options.revision && options.revision === stats.revision)
    ) {
      return stats;
    }
  }
...
exports.default = PCR;
exports.detectHandler = async (options) => {
  options = getOptions(options);
